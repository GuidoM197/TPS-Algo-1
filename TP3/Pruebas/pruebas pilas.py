...
class JuegoFlood:
    ...
    def deshacer(self):
        ...
        ultimo_flood = self.pila_aux.ver_tope() #Ultimo flood en juego

        if self.primer_deshacer and self.pila_rehacer.esta_vacia(): #Apilo el flood actual en pila rehacer
            self.pila_rehacer.apilar(ultimo_flood)
            self.primer_deshacer = False

        self.flood.grilla = self.pila_deshacer.ver_tope() #Cambio tablero por el movimiento anterior

        if not self.pila_deshacer.esta_vacia():

            if not verificar_existencia(copiar_pila(self.pila_rehacer), self.pila_deshacer.ver_tope()):
        ...
        print(f'Largo de deshacer es: {len(self.pila_deshacer)} \n')

        print(f'Largo de rehacer es: {len(self.pila_rehacer)} \n')
    ...
    def rehacer(self):
        ...
        if not self.pila_rehacer.esta_vacia() and ultimo_flood == self.pila_rehacer.ver_tope():
            self.pila_rehacer.desapilar()

        if not self.pila_rehacer.esta_vacia() and self.pila_rehacer.ver_tope() == self.tablero_inicial:
            self.pila_deshacer.apilar(self.pila_rehacer.desapilar())

        if not self.pila_rehacer.esta_vacia() and self.pila_rehacer.ver_tope() == self.flood.grilla:
            self.pila_deshacer.apilar(self.pila_rehacer.desapilar())
            se_deshizo = True

            if self.pila_rehacer.esta_vacia():
                self.flood.grilla = ultimo_flood
            else:
                self.flood.grilla = self.pila_rehacer.ver_tope()

        if not self.pila_rehacer.esta_vacia() and not se_deshizo:

            if not verificar_existencia(copiar_pila(self.pila_deshacer), self.pila_rehacer.ver_tope()):
                self.pila_deshacer.apilar(self.pila_rehacer.desapilar())
            else:
                self.pila_rehacer.desapilar()
