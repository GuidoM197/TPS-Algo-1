...
class JuegoFlood:
    ...
    def __init__(self, alto, ancho, n_colores):
        ...
        self.cambio_rehacer = False 
        self.primer_deshacer = True
        self.primer_rehacer = True
        self.se_deshizo = False
        self.se_rehizo = False
        self.cantidad_deshechos = 0
    ...
    def cambiar_color(self, color):
        ...
        self.pila_aux.apilar(self.flood.clonar())
        ...
        if not self.pila_rehacer.esta_vacia() and verificar_existencia(copiar_pila(self.pila_deshacer), self.flood.clonar()):
            self.pila_rehacer = Pila()
            self.cambio_rehacer = True
    ...
    def deshacer(self):
        ...
        ultimo_flood = self.pila_aux.ver_tope()


        if self.pila_deshacer.ver_tope() == self.pila_aux.ver_tope(): #En el caso de que se rehaga en el mov tope se deshace
            self.pila_deshacer.desapilar()
        

        if verificar_existencia(copiar_pila(self.pila_deshacer), self.flood.grilla):
            self.pila_deshacer.desapilar()


        if self.pila_rehacer.esta_vacia(): #Apilo el flood actual en pila rehacer
            self.pila_rehacer.apilar(ultimo_flood)
        ...
        if not self.pila_deshacer.esta_vacia():
            ...
            print(f'Largo de deshacer: {len(self.pila_deshacer)} \n')
            print(f'Largo de rehacer: {len(self.pila_rehacer)} \n')

        self.cantidad_deshechos += 1
    ...
    def rehacer(self):
        ...
        if self.pila_rehacer.ver_tope() == self.tablero_inicial: #Si es igual al tablero inicial no rehago el movimiento
            self.pila_deshacer.apilar(self.pila_rehacer.desapilar())


        if (not self.se_rehizo) and (self.pila_rehacer.ver_tope() == self.pila_aux.ver_tope()):
            self.flood.grilla = self.pila_rehacer.ver_tope()
            self.pila_rehacer.desapilar()
            self.se_rehizo = True


        if (not self.se_rehizo) and self.pila_rehacer.ver_tope() == self.flood.grilla: #Si el tope ya es igual al flood actual directamente lo mando a deshacer
            self.pila_deshacer.apilar(self.pila_rehacer.desapilar())
            self.se_rehizo = True


        if (not self.se_rehizo) and (not self.pila_rehacer.esta_vacia()): # Si no esta vacia para este punto hago el cambio
            self.flood.grilla = self.pila_rehacer.ver_tope()
            self.pila_deshacer.apilar(self.pila_rehacer.desapilar())
            self.se_rehizo = True
